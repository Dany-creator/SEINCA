...
class AccountMoveLine(models.Model):
    ...
    def _compute_currency_rate(self):
        """
        Calcula la tasa de conversión para las líneas.
        NO modificamos el comportamiento nativo, solo calculamos currency_rate
        basado en tax_today para propósitos de visualización.
        IMPORTANTE: NO depende de move_id.tax_today para evitar recálculos.
        """
        for line in self:
            # Usar la tasa de la factura para el campo currency_rate
    ...
    def write(self, vals):
        """
        Override write SOLO para calcular campos de referencia (ref_unit, subtotal_ref).
        NO modificamos price_unit para evitar recálculos de impuestos.
        """
        result = super(AccountMoveLine, self).write(vals)
        
        # Después de guardar, solo actualizamos campos de referencia
        for line in self:
            # NO modificar líneas de sección, nota, impuestos o redondeo
            if line.display_type in ('line_section', 'line_note', 'tax', 'rounding'):
                continue
                
            tax_today = line.move_id.tax_today if line.move_id.tax_today else 0.0
            values_to_update = {}
            
            # SOLO calcular ref_unit si se modificó price_unit
            # NO calcular price_unit desde ref_unit (eso causa el problema)
            if 'price_unit' in vals and tax_today > 0:
                ref_unit_calculated = round(line.price_unit / tax_today, 2)
                values_to_update['ref_unit'] = ref_unit_calculated
                values_to_update['subtotal_ref'] = round(ref_unit_calculated * line.quantity, 2)
    ...
    def create(self, vals_list):
        """
        Override create SOLO para calcular campos de referencia (ref_unit, subtotal_ref).
        NO modificamos price_unit para evitar recálculos de impuestos.
        """
        for vals in vals_list:
            ...
            if move_id:
                ...
                # SOLO calcular ref_unit si se proporciona price_unit
                # NO calcular price_unit desde ref_unit
                if 'price_unit' in vals and tax_today > 0:
                    vals['ref_unit'] = round(vals['price_unit'] / tax_today, 2)
                    quantity = vals.get('quantity', 1.0)
                    vals['subtotal_ref'] = round(vals['ref_unit'] * quantity, 2)
    ...
    @api.depends('amount_currency', 'debit', 'move_id.currency_id', 'company_id')
    def _debit_usd(self):
        """
        Calcula debit_usd desde amount_currency y tax_today.
        IMPORTANTE: NO depende de tax_today para evitar recálculos cuando cambie la tasa.
        Se calcula solo cuando cambian debit o amount_currency.
        """
        for rec in self:
            if not rec.debit == 0:
                tax_today = rec.tax_today if rec.tax_today else rec.move_id.tax_today
                if rec.move_id.currency_id == self.env.company.currency_id:
                    ...
                    rec.debit_usd = (amount_currency / tax_today) if tax_today > 0 else 0
                else:
                    rec.debit_usd = (rec.amount_currency if rec.amount_currency > 0 else (rec.amount_currency * -1))
            else:
                rec.debit_usd = 0

    @api.depends('amount_currency', 'credit', 'move_id.currency_id', 'company_id')
    def _credit_usd(self):
        """
        Calcula credit_usd desde amount_currency y tax_today.
        IMPORTANTE: NO depende de tax_today para evitar recálculos cuando cambie la tasa.
        Se calcula solo cuando cambian credit o amount_currency.
        """
        for rec in self:
            if not rec.credit == 0:
                tax_today = rec.tax_today if rec.tax_today else rec.move_id.tax_today
                if rec.move_id.currency_id == self.env.company.currency_id:
                    amount_currency = (rec.amount_currency if rec.amount_currency > 0 else (rec.amount_currency * -1))
                    rec.credit_usd = (amount_currency / tax_today) if tax_today > 0 else 0
                else:
