...
class PartnerLedgerCustomHandler(models.AbstractModel):
    ...
    def _get_initial_balance_values(self, partner_ids, options):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            # === CORRECCIÓN: Agregar '0 AS partial_id' a las consultas de balance inicial.
            if currency_dif == self.env.company.currency_id.symbol:
                queries.append(f"""
                    SELECT
                        account_move_line.partner_id,
                        0 AS partial_id,  -- ¡Corregido!
    ...
    def _get_query_sums(self, options):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            # No se necesita partial_id aquí, ya que solo se usan las sumas (debit, credit, balance).
            # Se mantiene el código original para este método.
            if currency_dif == self.env.company.currency_id.symbol:
                queries.append(f"""
                    SELECT
    ...
    def _get_aml_values(self, options, partner_ids, offset=0, limit=None):
        ...
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            # For the move lines directly linked to this partner (currency base)
            # === CORRECCIÓN: Se agrega '0 AS partial_id'
            if currency_dif == self.env.company.currency_id.symbol:
                queries.append(f'''
                    SELECT
                ...
                # For the move lines linked to no partner, but reconciled with this partner. They will appear in grey in the report (currency base)
                # === CORRECCIÓN: Se agrega 'partial.id AS partial_id'
                queries.append(f'''
                    SELECT
                        account_move_line.id,
                        account_move_line.date_maturity,
                        ...
                        account_move_line.amount_currency,
                        account_move_line.matching_number,
                        partial.id AS partial_id,  -- ¡Corregido!
                        COALESCE(account_move_line.invoice_date, account_move_line.date)     AS invoice_date,
                ...
                # For the move lines directly linked to this partner (USD currency)
                # === CORRECCIÓN: Se agrega '0 AS partial_id'
                queries.append(f'''
                                        SELECT
                                            account_move_line.id,
                ...
                # For the move lines linked to no partner, but reconciled with this partner. They will appear in grey in the report (USD currency)
                # === CORRECCIÓN: Se agrega 'partial.id AS partial_id'
                queries.append(f'''
                                        SELECT
                                            account_move_line.id,
